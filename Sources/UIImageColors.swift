...
extension UIImage {
    ...
    @objc(colorsOfImage)
    public func getColors() -> Array<UIColor> {
        var scaleDownSize: CGSize = self.size
        let quality = UIImageColorsQuality.high
        if quality != .highest {
            if self.size.width < self.size.height {
                let ratio = self.size.height/self.size.width
                scaleDownSize = CGSize(width: quality.rawValue/ratio, height: quality.rawValue)
        ...
        /*
         background: proposed[0].uicolor,
         primary: proposed[1].uicolor,
         secondary: proposed[1].uicolor,
         detail: proposed[3].uicolor
         */
        return [proposed[0].uicolor, proposed[1].uicolor, proposed[1].uicolor, proposed[3].uicolor]
    }
}
